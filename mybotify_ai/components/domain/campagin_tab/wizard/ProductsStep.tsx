...
export default function ProductsStep({ state, updateState, onNext, onBack, storeId }: ProductsStepProps) {
  ...
  const toggleProduct = (product: any) => {
    let selected = [];
    try {
      selected = JSON.parse(state.productsTargeted || "[]");
    } catch(e) {}
    ...
    updateState({ 
      productsTargeted: JSON.stringify(selected),
  ...
  const getSelectedIds = () => {
    try {
      return JSON.parse(state.productsTargeted || "[]").map((p: any) => p.id);
    } catch(e) {
      return [];
    }
  };
  ...
  return (
    <div className="flex flex-col h-full flex-1">
          ...
          <div className="max-h-[300px] overflow-y-auto border rounded p-2 grid grid-cols-2 gap-2">
            {products.map(p => (
              <div 
                ...
                className={`p-3 border rounded cursor-pointer ${selectedIds.includes(p.id) ? 'border-[#CAF389] bg-[#CAF389]/10' : 'hover:bg-gray-50'}`}
              >
      ...
      <div className="flex justify-between pt-4 border-t mt-4">
        <button onClick={onBack} className="px-4 py-2 border rounded text-gray-600 hover:bg-gray-100 font-semibold">
          Back
        </button>
        <button onClick={onNext} className="bg-[#2e3e48] text-white px-4 py-2 rounded font-semibold">
          Next Step
        </button>
      </div>
    </div>
